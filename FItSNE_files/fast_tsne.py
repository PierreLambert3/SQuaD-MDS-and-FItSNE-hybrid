...
def fast_tsne(
    X,
    method_type='fitsne', # must be in ['fitsne', 'hybrid', 'SQuaD_MDS']
    theta=0.5,
    hybrid_MDS_LR=0.5,
    hybrid_tsne_LR=1.,
    hybrid_niter_no_scale_fixing=150,
    perplexity=30,
    map_dims=2,
    max_iter=750,
    ...
    method_type: String, values can be: 'fitsne', 'hybrid', or 'SQuaD_MDS'
    ...
    infile = 'data_fitsne.txt'
    outfile = 'results_fitsne.txt'
    ...
    if method_type == 'hybrid':
        method_code = 1
    elif method_type == 'SQuaD_MDS':
        method_code = 2
    print("\n\n will run the following method: ", method_type, "(code: "+str(method_code)+")\n")
    # run the method
    subprocess.call(
        [
            ...
            outfile,
            "{}".format(nthreads),
            str(method_code), # '0' for fitsne, '1' for hybrid, '2' for SQuaD_MDS
            str(hybrid_MDS_LR),
            str(hybrid_tsne_LR),
            str(hybrid_niter_no_scale_fixing),
        ]
    )
    print("\n... DONE\n")
